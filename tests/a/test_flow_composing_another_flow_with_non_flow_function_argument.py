...
async def hello_world_greeting(index: int, greeting: FlowFunction[str]) -> str:
    result_greeting = f"{await greeting()} - {index}"
...
class TestFlowComposingAnotherFlowWithNonFlowFunctionArgument(
    unittest.IsolatedAsyncioTestCase
):
    async def test_flow_composing_another_flow_with_non_flow_function_argument(self):
        await hello_world()
