...
class TestFlowWithArgumentDefaultValue(unittest.IsolatedAsyncioTestCase):
    async def test_flow_with_argument_default_value(self):
        await hello_world()
        greet_using_greeting_mock.assert_called_once_with("Hello World!")

        greet_using_greeting_mock.reset_mock()

        await hello_world(greeting="Hello World2!")
