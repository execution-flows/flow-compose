...
@flow(
    ...
    index2=FlowArgument(int, default=13),
    greeting=greeting_hello_world,
)
async def hello_world_greeting(
    index: FlowArgument[int], index2: FlowArgument[int], greeting: FlowFunction[str]
) -> str:
    result_greeting = f"{await greeting()} - {await index()} - {await index2()}"
...
class TestFlowComposingAnotherFlowWithFlowArgumentDefaultValue(
    unittest.IsolatedAsyncioTestCase
):
    async def test_flow_composing_another_flow_with_flow_argument_default_value(
        self,
    ) -> None:
        await hello_world()
