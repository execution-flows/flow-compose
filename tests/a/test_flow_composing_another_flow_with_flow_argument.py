...
async def hello_world_greeting(
    index: FlowArgument[int], greeting: FlowFunction[str]
) -> str:
    result_greeting = f"{await greeting()} - {await index()}"
...
class TestFlowComposingAnotherFlowWithFlowArgument(unittest.IsolatedAsyncioTestCase):
    async def test_flow_composing_another_flow_with_flow_argument(self) -> None:
        await hello_world()
